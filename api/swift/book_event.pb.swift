// DO NOT EDIT.
// swift-format-ignore-file
//
// Generated by the Swift generator plugin for the protocol buffer compiler.
// Source: book_event.proto
//
// For information on using the generated types, please see the documentation:
//   https://github.com/apple/swift-protobuf/

import Foundation
import SwiftProtobuf

// If the compiler emits an error on this type, it is because this file
// was generated by a version of the `protoc` Swift plug-in that is
// incompatible with the version of SwiftProtobuf to which you are linking.
// Please ensure that you are building against the same version of the API
// that was used to generate this file.
fileprivate struct _GeneratedWithProtocGenSwiftVersion: SwiftProtobuf.ProtobufAPIVersionCheck {
  struct _2: SwiftProtobuf.ProtobufAPIVersion_2 {}
  typealias Version = _2
}

struct Bookowl_BookEventRegisterRequest {
  // SwiftProtobuf.Message conformance is added in an extension below. See the
  // `Message` and `Message+*Additions` files in the SwiftProtobuf library for
  // methods supported on all messages.

  var bookeventInfo: Bookowl_BookEventInfo {
    get {return _bookeventInfo ?? Bookowl_BookEventInfo()}
    set {_bookeventInfo = newValue}
  }
  /// Returns true if `bookeventInfo` has been explicitly set.
  var hasBookeventInfo: Bool {return self._bookeventInfo != nil}
  /// Clears the value of `bookeventInfo`. Subsequent reads from it will return its default value.
  mutating func clearBookeventInfo() {self._bookeventInfo = nil}

  var bookmarkID: String = String()

  var unknownFields = SwiftProtobuf.UnknownStorage()

  init() {}

  fileprivate var _bookeventInfo: Bookowl_BookEventInfo? = nil
}

struct Bookowl_BookEventRegisterResponse {
  // SwiftProtobuf.Message conformance is added in an extension below. See the
  // `Message` and `Message+*Additions` files in the SwiftProtobuf library for
  // methods supported on all messages.

  var bookeventID: String = String()

  var time: SwiftProtobuf.Google_Protobuf_Timestamp {
    get {return _time ?? SwiftProtobuf.Google_Protobuf_Timestamp()}
    set {_time = newValue}
  }
  /// Returns true if `time` has been explicitly set.
  var hasTime: Bool {return self._time != nil}
  /// Clears the value of `time`. Subsequent reads from it will return its default value.
  mutating func clearTime() {self._time = nil}

  var unknownFields = SwiftProtobuf.UnknownStorage()

  init() {}

  fileprivate var _time: SwiftProtobuf.Google_Protobuf_Timestamp? = nil
}

struct Bookowl_BookEventInfo {
  // SwiftProtobuf.Message conformance is added in an extension below. See the
  // `Message` and `Message+*Additions` files in the SwiftProtobuf library for
  // methods supported on all messages.

  var readStartTime: SwiftProtobuf.Google_Protobuf_Timestamp {
    get {return _readStartTime ?? SwiftProtobuf.Google_Protobuf_Timestamp()}
    set {_readStartTime = newValue}
  }
  /// Returns true if `readStartTime` has been explicitly set.
  var hasReadStartTime: Bool {return self._readStartTime != nil}
  /// Clears the value of `readStartTime`. Subsequent reads from it will return its default value.
  mutating func clearReadStartTime() {self._readStartTime = nil}

  var readEndTime: SwiftProtobuf.Google_Protobuf_Timestamp {
    get {return _readEndTime ?? SwiftProtobuf.Google_Protobuf_Timestamp()}
    set {_readEndTime = newValue}
  }
  /// Returns true if `readEndTime` has been explicitly set.
  var hasReadEndTime: Bool {return self._readEndTime != nil}
  /// Clears the value of `readEndTime`. Subsequent reads from it will return its default value.
  mutating func clearReadEndTime() {self._readEndTime = nil}

  var readStartPosition: Int64 = 0

  var readEndPosition: Int64 = 0

  var unknownFields = SwiftProtobuf.UnknownStorage()

  init() {}

  fileprivate var _readStartTime: SwiftProtobuf.Google_Protobuf_Timestamp? = nil
  fileprivate var _readEndTime: SwiftProtobuf.Google_Protobuf_Timestamp? = nil
}

// MARK: - Code below here is support for the SwiftProtobuf runtime.

fileprivate let _protobuf_package = "bookowl"

extension Bookowl_BookEventRegisterRequest: SwiftProtobuf.Message, SwiftProtobuf._MessageImplementationBase, SwiftProtobuf._ProtoNameProviding {
  static let protoMessageName: String = _protobuf_package + ".BookEventRegisterRequest"
  static let _protobuf_nameMap: SwiftProtobuf._NameMap = [
    1: .standard(proto: "bookevent_info"),
    2: .standard(proto: "bookmark_id"),
  ]

  mutating func decodeMessage<D: SwiftProtobuf.Decoder>(decoder: inout D) throws {
    while let fieldNumber = try decoder.nextFieldNumber() {
      // The use of inline closures is to circumvent an issue where the compiler
      // allocates stack space for every case branch when no optimizations are
      // enabled. https://github.com/apple/swift-protobuf/issues/1034
      switch fieldNumber {
      case 1: try { try decoder.decodeSingularMessageField(value: &self._bookeventInfo) }()
      case 2: try { try decoder.decodeSingularStringField(value: &self.bookmarkID) }()
      default: break
      }
    }
  }

  func traverse<V: SwiftProtobuf.Visitor>(visitor: inout V) throws {
    // The use of inline closures is to circumvent an issue where the compiler
    // allocates stack space for every if/case branch local when no optimizations
    // are enabled. https://github.com/apple/swift-protobuf/issues/1034 and
    // https://github.com/apple/swift-protobuf/issues/1182
    try { if let v = self._bookeventInfo {
      try visitor.visitSingularMessageField(value: v, fieldNumber: 1)
    } }()
    if !self.bookmarkID.isEmpty {
      try visitor.visitSingularStringField(value: self.bookmarkID, fieldNumber: 2)
    }
    try unknownFields.traverse(visitor: &visitor)
  }

  static func ==(lhs: Bookowl_BookEventRegisterRequest, rhs: Bookowl_BookEventRegisterRequest) -> Bool {
    if lhs._bookeventInfo != rhs._bookeventInfo {return false}
    if lhs.bookmarkID != rhs.bookmarkID {return false}
    if lhs.unknownFields != rhs.unknownFields {return false}
    return true
  }
}

extension Bookowl_BookEventRegisterResponse: SwiftProtobuf.Message, SwiftProtobuf._MessageImplementationBase, SwiftProtobuf._ProtoNameProviding {
  static let protoMessageName: String = _protobuf_package + ".BookEventRegisterResponse"
  static let _protobuf_nameMap: SwiftProtobuf._NameMap = [
    1: .standard(proto: "bookevent_id"),
    2: .same(proto: "time"),
  ]

  mutating func decodeMessage<D: SwiftProtobuf.Decoder>(decoder: inout D) throws {
    while let fieldNumber = try decoder.nextFieldNumber() {
      // The use of inline closures is to circumvent an issue where the compiler
      // allocates stack space for every case branch when no optimizations are
      // enabled. https://github.com/apple/swift-protobuf/issues/1034
      switch fieldNumber {
      case 1: try { try decoder.decodeSingularStringField(value: &self.bookeventID) }()
      case 2: try { try decoder.decodeSingularMessageField(value: &self._time) }()
      default: break
      }
    }
  }

  func traverse<V: SwiftProtobuf.Visitor>(visitor: inout V) throws {
    // The use of inline closures is to circumvent an issue where the compiler
    // allocates stack space for every if/case branch local when no optimizations
    // are enabled. https://github.com/apple/swift-protobuf/issues/1034 and
    // https://github.com/apple/swift-protobuf/issues/1182
    if !self.bookeventID.isEmpty {
      try visitor.visitSingularStringField(value: self.bookeventID, fieldNumber: 1)
    }
    try { if let v = self._time {
      try visitor.visitSingularMessageField(value: v, fieldNumber: 2)
    } }()
    try unknownFields.traverse(visitor: &visitor)
  }

  static func ==(lhs: Bookowl_BookEventRegisterResponse, rhs: Bookowl_BookEventRegisterResponse) -> Bool {
    if lhs.bookeventID != rhs.bookeventID {return false}
    if lhs._time != rhs._time {return false}
    if lhs.unknownFields != rhs.unknownFields {return false}
    return true
  }
}

extension Bookowl_BookEventInfo: SwiftProtobuf.Message, SwiftProtobuf._MessageImplementationBase, SwiftProtobuf._ProtoNameProviding {
  static let protoMessageName: String = _protobuf_package + ".BookEventInfo"
  static let _protobuf_nameMap: SwiftProtobuf._NameMap = [
    1: .standard(proto: "read_start_time"),
    2: .standard(proto: "read_end_time"),
    3: .standard(proto: "read_start_position"),
    4: .standard(proto: "read_end_position"),
  ]

  mutating func decodeMessage<D: SwiftProtobuf.Decoder>(decoder: inout D) throws {
    while let fieldNumber = try decoder.nextFieldNumber() {
      // The use of inline closures is to circumvent an issue where the compiler
      // allocates stack space for every case branch when no optimizations are
      // enabled. https://github.com/apple/swift-protobuf/issues/1034
      switch fieldNumber {
      case 1: try { try decoder.decodeSingularMessageField(value: &self._readStartTime) }()
      case 2: try { try decoder.decodeSingularMessageField(value: &self._readEndTime) }()
      case 3: try { try decoder.decodeSingularInt64Field(value: &self.readStartPosition) }()
      case 4: try { try decoder.decodeSingularInt64Field(value: &self.readEndPosition) }()
      default: break
      }
    }
  }

  func traverse<V: SwiftProtobuf.Visitor>(visitor: inout V) throws {
    // The use of inline closures is to circumvent an issue where the compiler
    // allocates stack space for every if/case branch local when no optimizations
    // are enabled. https://github.com/apple/swift-protobuf/issues/1034 and
    // https://github.com/apple/swift-protobuf/issues/1182
    try { if let v = self._readStartTime {
      try visitor.visitSingularMessageField(value: v, fieldNumber: 1)
    } }()
    try { if let v = self._readEndTime {
      try visitor.visitSingularMessageField(value: v, fieldNumber: 2)
    } }()
    if self.readStartPosition != 0 {
      try visitor.visitSingularInt64Field(value: self.readStartPosition, fieldNumber: 3)
    }
    if self.readEndPosition != 0 {
      try visitor.visitSingularInt64Field(value: self.readEndPosition, fieldNumber: 4)
    }
    try unknownFields.traverse(visitor: &visitor)
  }

  static func ==(lhs: Bookowl_BookEventInfo, rhs: Bookowl_BookEventInfo) -> Bool {
    if lhs._readStartTime != rhs._readStartTime {return false}
    if lhs._readEndTime != rhs._readEndTime {return false}
    if lhs.readStartPosition != rhs.readStartPosition {return false}
    if lhs.readEndPosition != rhs.readEndPosition {return false}
    if lhs.unknownFields != rhs.unknownFields {return false}
    return true
  }
}
